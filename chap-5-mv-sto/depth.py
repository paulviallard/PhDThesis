...
data = NDData("sourcecode/chap_5_depth.csv")
...
for k in range(len(data_dict.keys())):
    ...
    for learner in learner_dict.keys():
        ...
        tmp_data = tmp_data.astype({"max_depth": int}, errors='raise')
        max_depth_list = np.sort(
            np.unique(tmp_data["max_depth"].to_numpy()))
    ...
    fig.savefig("figures/depth_{}.pdf".format(j),
                bbox_inches="tight")
