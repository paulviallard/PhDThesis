...
nb_per_attribute_list = np.array([32, 64, 128])
...
for learner in learner_dict.keys():
    ...
    for nb_per_attribute in nb_per_attribute_list:

        data_list = ["moons-"+str(noise).replace(".", "_")+"-"+str(seed)
                     for seed in seed_list]
        learner_dict[learner]["nb_per_attribute"] = nb_per_attribute
    ...
    ax_list[0, 0].plot(
        nb_per_attribute_list, tS_mean, **learner_plot_dict[learner])
    ax_list[0, 0].fill_between(
        nb_per_attribute_list, tS_mean-tS_std,
        tS_mean+tS_std, alpha=0.1, **learner_fill_dict[learner])
    ax_list[0, 0].set_xlabel(r"Number of Decision Stumps per Feature")
    ax_list[0, 0].set_ylabel(
        r"Stochastic Risk $\EE_{(\x,\y)\sim\dS}s_{\hyperQ}(\x, \y)$")
    ax_list[0, 0].set_xlim(
        np.min(nb_per_attribute_list), np.max(nb_per_attribute_list))

    ax_list[0, 1].plot(
        nb_per_attribute_list, tT_mean, **learner_plot_dict[learner])
    ax_list[0, 1].fill_between(
        nb_per_attribute_list, tT_mean-tT_std, tT_mean+tT_std, alpha=0.1)
    ax_list[0, 1].set_xlabel(r"Number of Decision Stumps per Feature")
    ax_list[0, 1].set_ylabel(
        r"Stochastic Risk $\EE_{(\x,\y)\sim\dT}s_{\hyperQ}(\x, \y)$")
    ax_list[0, 1].set_xlim(
        np.min(nb_per_attribute_list), np.max(nb_per_attribute_list))

    ax_list[1, 0].plot(
        nb_per_attribute_list, bound_mean, **learner_plot_dict[learner])
    ax_list[1, 0].fill_between(
        nb_per_attribute_list,
        bound_mean-bound_std, bound_mean+bound_std, alpha=0.1)
    ax_list[1, 0].set_xlabel(r"Number of Decision Stumps per Feature")
    ax_list[1, 0].set_ylabel(r"Bound Value")
    ax_list[1, 0].set_xlim(
        np.min(nb_per_attribute_list), np.max(nb_per_attribute_list))

    ax_list[1, 1].plot(
        nb_per_attribute_list, time_mean, **learner_plot_dict[learner])
    ax_list[1, 1].fill_between(
        nb_per_attribute_list,
        time_mean-time_std, time_mean+time_std, alpha=0.1)
    ax_list[1, 1].set_xlabel(r"Number of Decision Stumps per Feature")
    ax_list[1, 1].set_ylabel(r"Time (in s)")
    ax_list[1, 1].set_xlim(
        np.min(nb_per_attribute_list), np.max(nb_per_attribute_list))
    ...
    fig.savefig("figures/moons_voter.pdf", bbox_inches="tight")
