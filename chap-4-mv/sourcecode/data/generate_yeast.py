...
def get_input(data):
    ...
    data_unique = list(np.unique(data[0]))
    data_unique.sort()
    for i, unique in enumerate(data_unique):
        data[0].loc[data[0] == unique] = i
...
def main():
    ...
    arg_parser = ArgumentParser(
        description="Generate yeast dataset")
    ...
    if(not(os.path.exists("data-yeast/"))
       or not(os.path.exists("data-yeast/yeast.data"))):
        ...
        f = open("data-yeast/yeast.data", "wb")
        f.write(r.content)
        f.close()

    # We open, process and save the dataset
    data = pd.read_csv(
        "data-yeast/yeast.data", sep=r"\s+", header=None)
    ...
    save("yeast.h5", input_train, input_test, label_train, label_test)
