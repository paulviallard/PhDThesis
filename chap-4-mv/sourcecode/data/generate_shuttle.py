...
def main():
    ...
    arg_parser = ArgumentParser(
        description="Generate shuttle dataset")
    arg_parser.parse_args()

    # We download the dataset
    if(not(os.path.exists("data-shuttle/"))
       or not(os.path.exists("data-shuttle/shuttle.scale"))
       or not(os.path.exists("data-shuttle/shuttle.scale.t"))):

        if(not(os.path.exists("data-shuttle"))):
            os.mkdir("data-shuttle")

        r = requests.get(
            "https://www.csie.ntu.edu.tw/~cjlin/libsvmtools/datasets/"
            + "multiclass/shuttle.scale",
            allow_redirects=True)
        f = open("data-shuttle/shuttle.scale", "wb")
        f.write(r.content)
        f.close()

        r = requests.get(
            "https://www.csie.ntu.edu.tw/~cjlin/libsvmtools/datasets/"
            + "multiclass/shuttle.scale.t",
            allow_redirects=True)
        f = open("data-shuttle/shuttle.scale.t", "wb")
        f.write(r.content)
        f.close()

    # We open, process and save the dataset
    data_list = load_svmlight_files(
        ["data-shuttle/shuttle.scale", "data-shuttle/shuttle.scale.t"])
    ...
    save("shuttle.h5", input_train, input_test, label_train, label_test)
