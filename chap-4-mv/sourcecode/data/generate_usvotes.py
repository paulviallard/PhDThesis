...
def get_label(data):
    ...
    """
    label = data[0]
    label.loc[label == "republican"] = -1
    label.loc[label == "democrat"] = 1
...
def main():
    ...
    arg_parser = ArgumentParser(
        description="Generate usvotes dataset")
    ...
    if(not(os.path.exists("data-usvotes/"))
       or not(os.path.exists("data-usvotes/usvotes.data"))):

        if(not(os.path.exists("data-usvotes"))):
            os.mkdir("data-usvotes")
        r = requests.get(
            "https://archive.ics.uci.edu/ml/machine-learning-databases/"
            + "voting-records/house-votes-84.data", allow_redirects=True)
        f = open("data-usvotes/usvotes.data", "wb")
        f.write(r.content)
        f.close()
    ...
    data = pd.read_csv(
        "data-usvotes/usvotes.data", sep=",", header=None)
    ...
    save("usvotes.h5", input_train, input_test, label_train, label_test)
