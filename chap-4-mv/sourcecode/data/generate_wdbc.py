...
def main():
    ...
    arg_parser = ArgumentParser(
        description="Generate wdbc dataset")
    ...
    if(not(os.path.exists("data-wdbc/"))
       or not(os.path.exists("data-wdbc/wdbc.data"))):
        ...
        r = requests.get(
            ...
            + "breast-cancer-wisconsin/wdbc.data", allow_redirects=True)
        f = open("data-wdbc/wdbc.data", "wb")
        f.write(r.content)
        f.close()
    ...
    save("wdbc.h5", input_train, input_test, label_train, label_test)
