...
def main():
    ...
    arg_parser = ArgumentParser(
        description="Generate haberman dataset")
    ...
    if(not(os.path.exists("data-haberman/"))
       or not(os.path.exists("data-tictactoe/haberman.data"))):

        if(not(os.path.exists("data-haberman"))):
            os.mkdir("data-haberman")

        r = requests.get(
            "https://archive.ics.uci.edu/ml/machine-learning-databases/"
            + "haberman/haberman.data", allow_redirects=True)
        f = open("data-haberman/haberman.data", "wb")
        f.write(r.content)
        f.close()

    data = pd.read_csv(
        "data-haberman/haberman.data", sep=",",
    ...
    save("haberman.h5", input_train, input_test, label_train, label_test)
