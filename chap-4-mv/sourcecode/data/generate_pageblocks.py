...
def main():
    ...
    if(not(os.path.exists("data-pageblocks/"))
       or not(os.path.exists("data-pageblocks/page-blocks.data"))):

        if(not(os.path.exists("data-pageblocks"))):
            os.mkdir("data-pageblocks")
        ...
        f = open("data-pageblocks/page-blocks.data.Z", "wb")
        f.write(r.content)
        f.close()
        os.system("uncompress data-pageblocks/page-blocks.data.Z")

    # We open, process and save the dataset
    data = pd.read_csv(
        "data-pageblocks/page-blocks.data", sep=r"\s+", header=None)
    ...
    save("pageblocks.h5", input_train, input_test, label_train, label_test)
