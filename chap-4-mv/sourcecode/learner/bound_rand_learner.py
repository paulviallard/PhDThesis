...
class BoundRandLearner(GradientDescentLearner):

    def __init__(
        self, majority_vote, epoch=1, m=1, batch_size=None, delta=0.05,
        rand_n=100, writer=None
    ):
        ...
        self.rand = Modules("Rand", self.mv_diff)
        self.bound = Modules(
            "BoundRand", self.mv_diff, m=self.m, delta=self.delta,
            rand_n=self.rand_n)
    ...
    def _optimize(self, batch):
        ...
        self._loss = bound
        rand = self.rand.fit(margin=margin)
