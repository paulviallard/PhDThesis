...
l1 = ax.plot(x, y_1, c=RED, label=r"$\loss(\h_{\S}, (\x, \y))$")
l2 = ax.plot(x, y_2, "--", c=BLUE, label=r"$\loss(\h_{\S}, (\x', \y'))$")
...
ax.text(x[i_max]+0.1,
        (np.minimum(y_1[i_max], y_2[i_max])
         + np.maximum(y_1[i_max], y_2[i_max]))/2.0+0.15, r"${\le}\rob$")

ax.set_xlabel(
    r"Learning samples $\S$ / Examples $(\x, \y)$ and $(\x', \y') \in \Z_i$")
ax.set_ylabel(
    r"Loss value")
...
fig.savefig("figures/robustness.pdf", bbox_inches="tight")
