...
def tree(x_):
    y_1 = (x_[:, 0] <= 0.59).astype(float)
    y_2 = (x_[:, 1] <= 0.63).astype(float)
    return y_1*y_2
...
ax.set_xticks([0.59])
ax.set_xticklabels([r"$0.59$"])
ax.set_yticks([0.63])
ax.set_yticklabels([r"$0.63$"])

ax.set_xlabel(r"$x_1$")
ax.set_ylabel(r"$x_2$")

os.makedirs("figures/", exist_ok=True)
fig.savefig("figures/tree.pdf", bbox_inches="tight")
